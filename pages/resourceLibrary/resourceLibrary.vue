<template>
	<view class="resL_content">
		<view class="fixedBox">
			<view class="unisearchBox">
				<view class="unisearch" @click="goSearch">
					<uni-search-bar cancelButton='none' bgColor='#F0F1F3' placeholder="搜索关键字" :radius="100" @confirm="search"></uni-search-bar>
				</view>
			</view>
			<view class="resf_card">
				<view class="resf_card_top" v-for="(f,i) in FileTypeList" :key='i' @click="onFileType(f.TypeKey)">
					<text>{{f.TypeName}}</text>|
				</view>
			</view>

		</view>
		<view class="" style="margin-bottom:220rpx;margin-top:280rpx;" @touchstart="onResourcesStart" @touchmove="onResourcesMove">
		...
		<view class="addFileBox">
			<view class="addFileBth" @click="UploadResources">
				添加资源
			</view>
		</view>
	...
	<view style="position: fixed;top: 0;bottom: 0;left: 0;right: 0;background: #fff; z-index: 20000000;" v-show="txtResult">
		<text>{{txtResult}}</text>
	</view>
	</view>
</template>
		...
		methods: {
			onResourcesStart(e) {
				...
				if (e.touches.length == 1) {
					...
					this.$data.ResourcesPageY = e.touches[0].pageY;
					console.log(this.$data.ResourcesPageY)
				}
			},
			onResourcesMove(e) {
				if (e.touches.length == 1) {
					...
					// console.log('moveY', moveY, 'disY', disY)
					if (this.$data.ResourcesPageY < 210) {
			...
			onFileType(TypeKey) {
				uni.navigateTo({
					url: '../ResourcesTag/ResourcesTag?FileType=' + TypeKey
				})
			},
			//标签管理
			...
			UploadResources() {
				...
				uni.navigateTo({
					url:`../uploadFile/uploadFile?name=https://hk.joyfulboo.com/Campus/ResourcesApp/UploadResources&tagid=0`
				})
				...
				// 		wx.uploadFile({
				// 			url: 'https://hk.joyfulboo.com/Campus/ResourcesApp/UploadResources',
			...
			GetEnumTypeInfo() {
				...
				uni.request({
					method: 'GET',
					url: 'https://hk.joyfulboo.com/Campus/ResourcesApp/GetEnumTypeInfo',
					data: {
						enumType: 'FileType'
					},
					header: {
					...
					success: (res) => {
						...
						if (result.Success == true) {
							_that.$data.FileTypeList = result.Data
						} else {
							uni.showToast({
